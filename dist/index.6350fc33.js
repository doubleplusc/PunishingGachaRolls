...
class Banner {
    ...
    checkPity(drop, category) {
        ...
        if (this.currentFiveStarPity > this.fiveStarPity || this.currentSixStarPity > this.sixStarPity) alert(`Pity had a nuclear meltdown. Please take a screenshot of your rolls and create an issue.`);
    }
    roll1() {
        let drop;
        let category;
        ...
        if (this.currentSixStarPity === this.sixStarPity) {
            drop = _itemDatabaseJs.database.pickOneFromCategory(this.sixStarPityType);
            category = this.sixStarPityType;
        } else if (this.currentFiveStarPity === this.fiveStarPity) {
            drop = _itemDatabaseJs.database.pickOneFromCategory(this.fiveStarPityType);
            ...
            drop = _itemDatabaseJs.database.pickOneFromCategory(category);
        }
    ...
    pickPity(pityCategory) {
        ...
        return _itemDatabaseJs.database.pickSpecificDrop();
        else if (this.rateUpSelection && !getSelectedRateUp) {
            //rate up is a lie
            let drop = _itemDatabaseJs.database.pickOneFromCategory(pityCategory);
            while(drop.name === this.rateUpSelection)drop = _itemDatabaseJs.database.pickOneFromCategoryWithoutRateUp();
        } else //no rate up
...
class BaseSelector {
    ...
    pickOneFromCategoryWithoutRateUp(rateUpSelection) {
        let drop = this.pick();
        while(drop.name === rateUpSelection)drop = this.pick();
        return this.packReturnObject(drop);
    }
}
